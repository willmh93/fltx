#!/usr/bin/env python3
"""Serve browser metrics assets and save generated outputs."""
...
class BrowserMetricsHandler(SimpleHTTPRequestHandler):
    def __init__(self, *args: object, **kwargs: object) -> None:
        super().__init__(*args, directory=str(ROOT), **kwargs)

    def log_message(self, format: str, *args: object) -> None:
        sys.stderr.write(f"[browser-metrics] {format % args}\n")
    ...
    def do_POST(self) -> None:
        if self.path in {"/save-metrics-file", "/save-benchmark-file"}:
            self.save_metrics_file()
            return
        if self.path in {"/build-table", "/collect-table"}:
            self.build_table()
            return
    ...
    def save_metrics_file(self) -> None:
        try:
...
def main() -> None:
    ...
    server = ThreadingHTTPServer(("127.0.0.1", port), BrowserMetricsHandler)
    print(f"Serving fltx browser metrics at http://127.0.0.1:{port}/", flush=True)
