...
- Fixed-size extended-precision scalar types: `bl::f128` and `bl::f256`
- Native aliases for `bl::f32` and `bl::f64`
- `constexpr` arithmetic, comparisons, conversions, parsing, formatting, and math functions
- `<cmath>`-style API for generic numeric code
- Standard-library integration for streams, `std::numeric_limits`, `std::numbers`, and common stream manipulators
- Bitwise runtime/`constexpr` parity for `bl::f128` and `bl::f256` by default
...
- Tested and benchmarked against `boost::multiprecision::mpfr_float_backend<>` at comparable precision

## Core Types

| Type | Representation | Tested accuracy |
|---|---|---|
| `bl::f32` | Alias for native `float` | Native `float` precision |
| `bl::f64` | Alias for native `double` | Native `double` precision |
| `bl::f128` | Double-double, stored as two `double` limbs | Minimum 29 decimal digits across arithmetic and math functions |
| `bl::f256` | Quad-double, stored as four `double` limbs | Minimum 59 decimal digits across arithmetic and math functions |
...
`bl::f128` and `bl::f256` are not IEEE binary128 / binary256 types, and they are not arbitrary-precision numbers. They provide more precision than native `double` while preserving a fixed-width, scalar-friendly floating-point model. Values are still approximate, so conditioning, cancellation, argument reduction, and algorithm design still matter.
...
[`fltx_math.h`](include/fltx_math.h) provides a `constexpr` `<cmath>`-style API across `bl::f32`, `bl::f64`, `bl::f128`, and `bl::f256`.
...
For `bl::f32` and `bl::f64`, the default runtime path follows native `std::` performance. Define [`FLTX_CONSTEXPR_PARITY`](include/fltx_math.h) when you want the runtime implementation to match the `constexpr` path more closely.
...
This lets runtime values such as `FloatType::F128` or `FloatType::F256` select a compile-time type, so the called function still compiles as a normal template specialization.
