/**
 * fltx/detail/f128/math/trig.h - trigonometry implementation details.
 *
...
#ifndef FLTX_F128_DETAIL_TRIG_IMPL_INCLUDED
#define FLTX_F128_DETAIL_TRIG_IMPL_INCLUDED
...
namespace detail::_f128
{
    ...
    BL_FORCE_INLINE constexpr f128_s _asin(const f128_s& x)
    {
        ...
        if (ax <= f128_s{ 0.5 })
            return _atan(div_inline(x, detail::_f128_constexpr::sqrt(sub_inline(f128_s{ 1.0 }, mul_inline(x, x)))));

        const f128_s t = detail::_f128_constexpr::sqrt(div_inline(sub_double_inline(1.0, ax), add_double_inline(ax, 1.0)));
...
[[nodiscard]] BL_FORCE_INLINE constexpr f128_s detail::_f128_constexpr::atan2(const f128_s& y, const f128_s& x)
{
    ...
    {
        if (x.hi < 0.0)
            return signbit_constexpr(y.hi) ? -std::numbers::pi_v<f128_s> : std::numbers::pi_v<f128_s>;
        return y;
    }
