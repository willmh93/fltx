/**
 * fltx/f128/string.h - Constexpr string formatting, parsing, and literals for f128.
 *
...
#include "fltx/f128/limits.h"
#include "fltx/f128/rounding.h"
#include "fltx/detail/common/io.h"
...
namespace detail::_f128
{
    ...
    BL_FORCE_INLINE constexpr f128_chars_result emit_scientific_sig_to_chars_f128(char* first, char* last, const f128_s& x, int sig_digits, bool strip_trailing_zeros) noexcept
    {
        if (iszero(x)) {
            if (first >= last) return { first, false };
            *first = '0';
    ...
    BL_FORCE_INLINE constexpr f128_chars_result emit_scientific_to_chars(char* first, char* last, const f128_s& x, int frac_digits, bool strip_trailing_zeros) noexcept
    {
