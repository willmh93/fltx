...
constexpr const char* kTypeName = "f256";
...
[[nodiscard]] constexpr value_type canonicalization_canary() noexcept
{
    value_type value{
        1.0,
        0x1.0p-60,
...
TEST_CASE("f256 constexpr parity harness detects x3 low-bit differences", "[fltx][constexpr][parity][f256][harness]")
{
    const auto constexpr_result = eval_constexpr_path([]()
    {
        return canonicalization_canary();
    });

    const auto runtime_result = eval_runtime_path([]()
    {
        return canonicalization_canary();
    });
    ...
    std::fputs("[fltx parity] f256 harness low-bit canary passed\n", stderr);
    std::fflush(stderr);
}

TEST_CASE("f256 constexpr parity harness switches forced path", "[fltx][constexpr][parity][f256][harness]")
{
    ...
    std::fputs("[fltx parity] f256 harness forced path passed\n", stderr);
    std::fflush(stderr);
}
