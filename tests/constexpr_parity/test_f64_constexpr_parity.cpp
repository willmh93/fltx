...
constexpr const char* kTypeName = "f64";
...
static_assert(std::is_same_v<decltype(bl::pow(2.0, 5)), bl::f64>);
static_assert(std::is_same_v<decltype(bl::pow(2.0, 5.0f)), bl::f64>);
static_assert(bl::pow(2.0, 5) == 32.0);
static_assert(bl::pow(2.0, 5.0f) == 32.0);
static_assert(!can_call_pow<bl::f64, long double>);
...
[[nodiscard]] auto gen_pow_float_args(std::mt19937_64& rng, int bucket)
{
    return std::tuple{ gen_positive_nonzero_value(rng, bucket), static_cast<float>(gen_any_value(rng, bucket)) };
}
...
TEST_CASE("f64 constexpr parity: pow(float)", "[fltx][constexpr][parity][f64][pow_float]")
{
    run_tuple_test("pow_float", gen_pow_float_args, [](double x, float y) { return bl::pow(x, y); });
}

TEST_CASE("f64 constexpr parity: pow(int)", "[fltx][constexpr][parity][f64][pow_int]")
{
