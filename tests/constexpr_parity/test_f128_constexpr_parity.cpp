...
[[nodiscard]] long long random_long_long(bl::mt19937_64& rng, long long lo, long long hi) noexcept
{
    return bl::uniform_int_distribution<long long>(lo, hi)(rng);
}
...
[[nodiscard]] sample_type gen_any_value(bl::mt19937_64& rng, int bucket)
{
    ...
    case 1:
        return compose_terms(rng, 2, -250, 250, 30, 60);

    case 2:
        return compose_terms(rng, 2, -500, 500, 2, 20);

    default:
    {
        const int lead_exp = random_int(rng, -950, 950);
        sample_type lead = scaled_value(rng, lead_exp, lead_exp);
        const int tail_exp = lead_exp - random_int(rng, 40, 100);
        sample_type tail = scaled_value(rng, tail_exp, tail_exp);
        return (lead + tail) - lead;
    }
    }
}
...
{
    sample_type value = signed_unit(rng) * sample_type{ bucket == 0 ? 0.75 : 0.95 };
    if (bucket >= 1)
        value += signed_unit(rng) * sample_type{ std::ldexp(1.0, -40) };
    if (bucket >= 2)
        value += signed_unit(rng) * sample_type{ std::ldexp(1.0, -80) };
    if (bucket >= 3)
        value = (sample_type{ 1.0 } + value) - sample_type{ 1.0 };
...
[[nodiscard]] sample_type gen_gamma_value(bl::mt19937_64& rng, int bucket)
{
    ...
    if (bucket >= 2)
        value += signed_unit(rng) * sample_type{ std::ldexp(1.0, -80) };
    if (bucket >= 3)
        value = (sample_type{ 2.0 } + value) - sample_type{ 2.0 };
...
[[nodiscard]] int gen_round_digits_value(bl::mt19937_64& rng, int bucket) noexcept
{
    ...
    case 2: return random_int(rng, -40, 40);
    default: return random_int(rng, -60, 60);
    }
}
...
{
    if (double_has_constexpr_dekker_overflow_risk(value.hi))
        return true;

    if (double_has_constexpr_dekker_overflow_risk(value.lo))
        return true;
...
[[nodiscard]] bool value_has_constexpr_recip_dekker_overflow_risk(const value_type& value) noexcept
{
    ...
    if (double_recip_estimate_has_constexpr_dekker_overflow_risk(value.hi))
        return true;
...
{
    return std::bit_cast<std::uint64_t>(lhs.hi) == std::bit_cast<std::uint64_t>(rhs.hi) &&
           std::bit_cast<std::uint64_t>(lhs.lo) == std::bit_cast<std::uint64_t>(rhs.lo);
}
...
TEST_CASE("f128 constexpr parity: clamp", "[fltx][constexpr][parity][f128][clamp]")
{
...
TEST_CASE("f128 constexpr parity: pow", "[fltx][constexpr][parity][f128][pow]")
{
...
TEST_CASE("f128 constexpr parity: frexp", "[fltx][constexpr][parity][f128][frexp]")
{
...
TEST_CASE("f128 constexpr parity: modf", "[fltx][constexpr][parity][f128][modf]")
{
...
TEST_CASE("f128 constexpr parity harness switches forced path", "[fltx][constexpr][parity][f128][harness]")
{
