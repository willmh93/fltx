...
namespace
{
    ...
    struct generated_binary_value_spec
    {
        std::string lhs{};
        std::string rhs{};
    };
    ...
    struct generated_recurrence_value_spec
    {
        std::string x{};
        std::string y{};
        std::string a{};
    ...
    struct generated_ldexp_value_spec
    {
        std::string value{};
        int exponent = 0;
    };
    ...
    [[nodiscard]] const char* benchmark_text(const std::string& text)
    {
        return text.c_str();
    }
    ...
    void observe_random_value_domain(random_value_domain& domain, const char* text)
    {
        char* end = nullptr;
    ...
    void hash_spec(std::uint64_t& hash, const char* text)
    {
        hash_text(hash, text == nullptr ? std::string_view{} : std::string_view{text});
    }
    ...
    [[nodiscard]] char random_digit(std::mt19937_64& rng)
    {
        return static_cast<char>('0' + std::uniform_int_distribution<int>{0, 9}(rng));
    }

    [[nodiscard]] char random_nonzero_digit(std::mt19937_64& rng)
    {
        return static_cast<char>('1' + std::uniform_int_distribution<int>{0, 8}(rng));
    }

    void append_random_digits(std::string& text, std::mt19937_64& rng, std::size_t count)
    {
        for (std::size_t i = 0; i < count; ++i)
            text.push_back(random_digit(rng));
    }

    [[nodiscard]] bool choose_negative(std::mt19937_64& rng, const random_value_domain& domain)
    {
        if (!domain.saw_negative)
            return false;
        if (!domain.saw_positive)
            return true;
        return std::bernoulli_distribution{0.5}(rng);
    }

    [[nodiscard]] std::string make_unit_random_value(std::mt19937_64& rng, const random_value_domain& domain)
    {
        std::string text;
        if (choose_negative(rng, domain))
            text.push_back('-');

        text += "0.";
        text.push_back(random_nonzero_digit(rng));
        append_random_digits(text, rng, typical_precision_digits - 1);
        return text;
    }

    [[nodiscard]] std::string make_fixed_random_value(std::mt19937_64& rng, random_value_domain domain)
    {
        ...
        if (!domain.saw_negative)
            lo = std::max(lo, std::max(domain.min_abs, std::numeric_limits<double>::min()));

        double value = std::uniform_real_distribution<double>{lo, hi}(rng);
        if (!domain.saw_negative && value <= 0.0)
            value = std::max(domain.min_abs, std::numeric_limits<double>::min());
        if (value <= lo && lo < hi)
            value = std::nextafter(lo, hi);
        if (value >= hi && lo < hi)
            value = std::nextafter(hi, lo);
        ...
        {
            text.push_back('.');
            append_random_digits(text, rng, typical_precision_digits);
        }
        else
        {
            const std::size_t current_digits = text.size() - dot - 1;
            if (current_digits < typical_precision_digits)
                append_random_digits(text, rng, typical_precision_digits - current_digits);
        }

        return text;
    }

    [[nodiscard]] std::string make_scientific_random_value(std::mt19937_64& rng, random_value_domain domain)
    {
        const double min_abs = std::max(domain.min_abs, std::numeric_limits<double>::min());
        const double max_abs = std::max(domain.max_abs, min_abs);
        int min_exp10 = static_cast<int>(std::floor(std::log10(min_abs)));
        int max_exp10 = static_cast<int>(std::floor(std::log10(max_abs)));

        if (!domain.saw_negative && domain.min_value >= 1.0)
            min_exp10 = std::max(min_exp10, 0);
        if (min_exp10 > max_exp10)
            std::swap(min_exp10, max_exp10);

        min_exp10 = std::clamp(min_exp10, -300, 300);
        max_exp10 = std::clamp(max_exp10, -300, 300);
        if (min_exp10 > max_exp10)
            std::swap(min_exp10, max_exp10);
        ...
        append_random_digits(text, rng, typical_precision_digits);
        text.push_back('e');
        text += std::to_string(std::uniform_int_distribution<int>{min_exp10, max_exp10}(rng));
        return text;
    }

    [[nodiscard]] std::string make_random_benchmark_value(std::mt19937_64& rng, random_value_domain domain)
    {
        domain = normalized_random_value_domain(domain);

        if (domain.min_value >= -1.0 && domain.max_value <= 1.0)
            return make_unit_random_value(rng, domain);

        if (domain.max_abs <= 1'000'000.0)
            return make_fixed_random_value(rng, domain);
    ...
    [[nodiscard]] random_value_domain make_text_domain(const std::array<const char*, bucket_value_count>& texts)
    {
        ...
        for (const char* text : texts)
            observe_random_value_domain(domain, text);
    ...
    [[nodiscard]] std::array<std::string, typical_value_count> make_typical_value_specs(const bucket_array_set<const char*>& specs)
    {
        std::array<std::string, typical_value_count> out{};
        std::mt19937_64 rng{make_typical_seed(specs, "value")};
        const random_value_domain domain = make_text_domain(specs.medium);

        for (auto& text : out)
            text = make_random_benchmark_value(rng, domain);
    ...
    [[nodiscard]] std::array<generated_ldexp_value_spec, typical_value_count> make_typical_ldexp_specs(const bucket_array_set<ldexp_value_spec>& specs)
    {
        std::array<generated_ldexp_value_spec, typical_value_count> out{};
        ...
        for (auto& spec : out)
        {
            spec.value = make_random_benchmark_value(rng, value_domain);
            spec.exponent = std::uniform_int_distribution<int>{min_exponent, max_exponent}(rng);
        }

        return out;
    }

    [[nodiscard]] std::array<generated_recurrence_value_spec, typical_value_count> make_typical_recurrence_specs(const bucket_array_set<recurrence_value_spec>& specs)
    {
        std::array<generated_recurrence_value_spec, typical_value_count> out{};
        ...
        for (auto& spec : out)
        {
            ...
            spec.a = make_random_benchmark_value(rng, a_domain);
            spec.b = make_random_benchmark_value(rng, b_domain);
            spec.c = make_random_benchmark_value(rng, c_domain);
            spec.d = make_random_benchmark_value(rng, d_domain);
        }
    ...
    template<typename T, typename Spec, std::size_t ValueCount, typename Op>
        ...
        bool scale_to_bucket = true)
    {
        ...
        for (std::size_t i = 0; i < ValueCount; ++i)
        {
            values[i].first = parse_benchmark_value<T>(benchmark_text(specs[i].lhs));
            values[i].second = parse_benchmark_value<T>(benchmark_text(specs[i].rhs));
        }
        ...
        bool scale_to_bucket = true)
    {
        ...
        std::array<ldexp_case, ValueCount> values{};
        for (std::size_t i = 0; i < ValueCount; ++i)
        {
            values[i].value = parse_benchmark_value<T>(benchmark_text(specs[i].value));
        ...
        bool scale_to_bucket = true)
    {
        ...
        std::array<recurrence_case, ValueCount> values{};
        for (std::size_t i = 0; i < ValueCount; ++i)
        {
            values[i].x = parse_benchmark_value<T>(benchmark_text(specs[i].x));
            values[i].y = parse_benchmark_value<T>(benchmark_text(specs[i].y));
            values[i].a = parse_benchmark_value<T>(benchmark_text(specs[i].a));
            values[i].b = parse_benchmark_value<T>(benchmark_text(specs[i].b));
            values[i].c = parse_benchmark_value<T>(benchmark_text(specs[i].c));
            values[i].d = parse_benchmark_value<T>(benchmark_text(specs[i].d));
        }
        ...
        return run_benchmark(iteration_count, [&]()
        {
            ...
            for (std::int64_t outer = 0; outer < outer_loops; ++outer)
            {
                for (auto& item : state)
                {
                    auto xx = item.x * item.x;
                    auto yy = item.y * item.y;
                    auto xy0 = item.x * item.y;
                    auto xy1 = item.x * item.y;

                    const T next_x = (std::move(xx) - std::move(yy)) + item.a;
                    item.y = (std::move(xy0) + std::move(xy1)) + item.b;
                    item.x = next_x;
    ...
    template<typename T, typename Result, typename Spec, std::size_t ValueCount, typename Op>
        ...
        bool scale_to_bucket = true)
    {
        ...
        std::array<nexttoward_case, ValueCount> values{};
        for (std::size_t i = 0; i < ValueCount; ++i)
        {
            values[i].from = parse_benchmark_value<T>(benchmark_text(specs[i].lhs));
            const T target = parse_benchmark_value<T>(benchmark_text(specs[i].rhs));
    ...
    template<typename T, typename Result, typename Exponent, typename Spec, std::size_t ValueCount, typename Op>
        ...
        bool scale_to_bucket = true)
    {
        ...
        std::array<exponent_case, ValueCount> values{};
        for (std::size_t i = 0; i < ValueCount; ++i)
        {
            values[i].value = parse_benchmark_value<T>(benchmark_text(specs[i].value));
        ...
        bool scale_to_bucket = true)
    {
        ...
        std::array<ternary_case, ValueCount> values{};
        for (std::size_t i = 0; i < ValueCount; ++i)
        {
            values[i].x = parse_benchmark_value<T>(benchmark_text(specs[i].x));
            values[i].y = parse_benchmark_value<T>(benchmark_text(specs[i].y));
            values[i].z = parse_benchmark_value<T>(benchmark_text(specs[i].a));
        }
        ...
        Op&& op)
    {
        ...
        out.typical.f256 = benchmark_binary_bucket_result<f256, FltxResult>(typical_specs, total_iterations, op, false);
        ...
        Op&& op)
    {
        ...
        const auto typical_specs = make_typical_binary_specs(specs);
        out.typical.f256 = benchmark_binary_long_double_bucket_result<f256, FltxResult>(typical_specs, total_iterations, op, false);
        ...
        Op&& op)
    {
        ...
        out.typical.f256 = benchmark_exponent_bucket_result<f256, FltxResult, Exponent>(typical_specs, total_iterations, op, false);
        ...
        Op&& op)
    {
        ...
        out.typical.f256 = benchmark_ternary_bucket_result<f256, FltxResult>(typical_specs, total_iterations, op, false);
        ...
        Op&& op)
    {
        ...
        const auto typical_specs = make_typical_value_specs(specs);
        out.typical.f256 = benchmark_value_bucket<f256>(typical_specs, total_iterations, op, false);
        out.typical.mpfr = benchmark_value_bucket<mpfr_ref>(typical_specs, total_iterations, op, false);
        ...
        Op&& op)
    {
        ...
        const auto typical_specs = make_typical_binary_specs(specs);
        out.typical.f256 = benchmark_binary_bucket<f256>(typical_specs, total_iterations, op, false);
        ...
        std::int64_t total_iterations)
    {
        ...
        const auto typical_specs = make_typical_ldexp_specs(specs);
        out.typical.f256 = benchmark_ldexp_bucket<f256>(typical_specs, total_iterations, false);
        ...
        std::int64_t total_iterations)
    {
        ...
        if constexpr (!only_bench_typical)
        {
            out.easy.f256 = benchmark_scalar_mixed_recurrence_bucket<f256>(specs.easy, total_iterations);
            out.easy.mpfr = benchmark_scalar_mixed_recurrence_bucket<mpfr_ref>(specs.easy, total_iterations);
            out.medium.f256 = benchmark_scalar_mixed_recurrence_bucket<f256>(specs.medium, total_iterations);
            out.medium.mpfr = benchmark_scalar_mixed_recurrence_bucket<mpfr_ref>(specs.medium, total_iterations);
            out.hard.f256 = benchmark_scalar_mixed_recurrence_bucket<f256>(specs.hard, total_iterations);
            out.hard.mpfr = benchmark_scalar_mixed_recurrence_bucket<mpfr_ref>(specs.hard, total_iterations);
        }
        const auto typical_specs = make_typical_recurrence_specs(specs);
        out.typical.f256 = benchmark_scalar_mixed_recurrence_bucket<f256>(typical_specs, total_iterations, false);
        ...
        std::int64_t total_iterations)
    {
        ...
        if constexpr (!only_bench_typical)
        {
            out.easy.f256 = benchmark_fused_mixed_expression_bucket<f256>(specs.easy, total_iterations);
            out.easy.mpfr = benchmark_fused_mixed_expression_bucket<mpfr_ref>(specs.easy, total_iterations);
            out.medium.f256 = benchmark_fused_mixed_expression_bucket<f256>(specs.medium, total_iterations);
            out.medium.mpfr = benchmark_fused_mixed_expression_bucket<mpfr_ref>(specs.medium, total_iterations);
            out.hard.f256 = benchmark_fused_mixed_expression_bucket<f256>(specs.hard, total_iterations);
            out.hard.mpfr = benchmark_fused_mixed_expression_bucket<mpfr_ref>(specs.hard, total_iterations);
        }
        const auto typical_specs = make_typical_recurrence_specs(specs);
        out.typical.f256 = benchmark_fused_mixed_expression_bucket<f256>(typical_specs, total_iterations, false);
    ...
    [[nodiscard]] mandelbrot_kernel_total<T> run_mandelbrot_kernel()
    {
        ...
        const T center_x = parse_benchmark_value<T>("-1.73200006480238126967529761198455");
        const T center_y = parse_benchmark_value<T>("0.00000019235376499049335337716270");
        const T zoom = parse_benchmark_value<T>("2.0e+28");
