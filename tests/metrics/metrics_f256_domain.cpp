...
#define FLTX_F256_UNARY_DOMAIN(NAME, TAGS, REQUIRED_BITS, SAMPLES, CALL) \
    TEST_CASE("f256 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " #NAME " domain", "[metrics][domain][fltx][f256][primary]" TAGS) \
    { \
...
#define FLTX_F256_BINARY_DOMAIN(NAME, TAGS, REQUIRED_BITS, SAMPLES, CALL, REF_CALL) \
    TEST_CASE("f256 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " #NAME " domain", "[metrics][domain][fltx][f256][primary]" TAGS) \
    { \
        if (!bl::test::metrics::metrics_case_phase_enabled("domain")) { SUCCEED("metrics domain phase not selected"); return; } \
...
#define FLTX_F256_BINARY_BOOL_DOMAIN(NAME, LABEL, TAGS, REQUIRED_BITS, SAMPLES, CALL, REF_CALL) \
    TEST_CASE("f256 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " LABEL " domain", "[metrics][domain][fltx][f256][primary]" TAGS) \
    { \
        if (!bl::test::metrics::metrics_case_phase_enabled("domain")) { SUCCEED("metrics domain phase not selected"); return; } \
        ...
        const auto record = run_binary_bool_case<FLTX_PRIMARY_CASE_QDPP_EXTRA_SUPPORTED(NAME)>( \
            ...
            [](const auto& x, const auto& y) { return REF_CALL(x, y); }, \
            false, false); \
        bl::test::metrics::write_metrics_case_report("f256 primary domain: " LABEL, record); \
        CHECK(record.fltx_accuracy.sample_count > 0); \
    }

#define FLTX_F256_TERNARY_DOMAIN(NAME, TAGS, REQUIRED_BITS, SAMPLES, CALL, REF_CALL) \
    TEST_CASE("f256 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " #NAME " domain", "[metrics][domain][fltx][f256][primary]" TAGS) \
    { \
        if (!bl::test::metrics::metrics_case_phase_enabled("domain")) { SUCCEED("metrics domain phase not selected"); return; } \
...
#define FLTX_F256_UNARY_INT_DOMAIN(NAME, TAGS, REQUIRED_BITS, SAMPLES, CALL) \
    TEST_CASE("f256 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " #NAME " domain", "[metrics][domain][fltx][f256][primary]" TAGS) \
    { \
        if (!bl::test::metrics::metrics_case_phase_enabled("domain")) { SUCCEED("metrics domain phase not selected"); return; } \
...
#define FLTX_F256_UNARY_INTEGER_DOMAIN(NAME, TAGS, REQUIRED_BITS, SAMPLES, CALL, REF_CALL) \
    TEST_CASE("f256 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " #NAME " domain", "[metrics][domain][fltx][f256][primary]" TAGS) \
    { \
        if (!bl::test::metrics::metrics_case_phase_enabled("domain")) { SUCCEED("metrics domain phase not selected"); return; } \
        const auto samples = make_domain_unary_samples(#NAME); \
...
#define FLTX_F256_FREXP_DOMAIN(NAME, TAGS, REQUIRED_BITS, SAMPLES, CALL) \
    TEST_CASE("f256 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " #NAME " domain", "[metrics][domain][fltx][f256][primary]" TAGS) \
    { \
        if (!bl::test::metrics::metrics_case_phase_enabled("domain")) { SUCCEED("metrics domain phase not selected"); return; } \
        const auto samples = make_domain_unary_samples(#NAME); \
...
FLTX_F128_PRIMARY_CASES(
    FLTX_F256_UNARY_DOMAIN,
    FLTX_F256_BINARY_DOMAIN,
    FLTX_F256_TERNARY_DOMAIN,
    FLTX_F256_UNARY_INT_DOMAIN,
    FLTX_F256_UNARY_INTEGER_DOMAIN,
    FLTX_F256_FREXP_DOMAIN)

FLTX_F128_PRIMARY_COMPARISON_CASES(FLTX_F256_BINARY_BOOL_DOMAIN)

#undef FLTX_F256_UNARY_DOMAIN
#undef FLTX_F256_BINARY_DOMAIN
#undef FLTX_F256_BINARY_BOOL_DOMAIN
#undef FLTX_F256_TERNARY_DOMAIN
#undef FLTX_F256_UNARY_INT_DOMAIN
#undef FLTX_F256_UNARY_INTEGER_DOMAIN
#undef FLTX_F256_FREXP_DOMAIN
