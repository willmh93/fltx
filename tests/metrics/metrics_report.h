...
namespace bl::test::metrics
{
    ...
    {
        return record.competitor_supported && record.fltx_benchmark.ns_per_iter > 0.0
    ...
    {
        return fltx_benchmark.ns_per_iter > 0.0
        ...
        const metrics_record& record) noexcept
    {
        ...
        {
            if (!candidate.supported || candidate.accuracy == nullptr || candidate.benchmark == nullptr)
                return;
    ...
    private:
        ...
        void write_detail_border()
        {
            ...
            write_section_boundary();
            write_background_repeated(primary_competitor_background, '-', pair_group_width + 2);
            write_border_join(primary_competitor_background);
            write_background_repeated(primary_competitor_background, '-', domain_single_group_width + 2);
            write_border_join(primary_competitor_background);
            write_background_repeated(primary_competitor_background, '-', benchmark_single_group_width + 2);
            write_border_join(primary_competitor_background);
            write_background_repeated(primary_competitor_background, '-', special_single_group_width + 2);
            for (std::size_t index = 0; index < extra_competitors.size(); ++index)
            {
                write_section_boundary();
                write_background_repeated(extra_competitor_background, '-', pair_group_width + 2);
                write_border_join(extra_competitor_background);
                write_background_repeated(extra_competitor_background, '-', domain_single_group_width + 2);
                write_border_join(extra_competitor_background);
                write_background_repeated(extra_competitor_background, '-', benchmark_single_group_width + 2);
                write_border_join(extra_competitor_background);
                write_background_repeated(extra_competitor_background, '-', special_single_group_width + 2);
            }
            write_section_boundary();
            write_background_repeated(fltx_background, '-', pair_group_width + 2);
            write_border_join(fltx_background);
            write_background_repeated(fltx_background, '-', domain_single_group_width + 2);
            write_border_join(fltx_background);
            write_background_repeated(fltx_background, '-', benchmark_single_group_width + 2);
            write_border_join(fltx_background);
            write_background_repeated(fltx_background, '-', special_single_group_width + 2);
            write_section_boundary();
            write_background_repeated(fltx_background, '-', comparison_bits_group_width);
            write_border_join(fltx_background);
            write_background_repeated(fltx_background, '-', domain_single_group_width + 2);
            write_border_join(fltx_background);
            write_background_repeated(fltx_background, '-', benchmark_speed_width + 2);
            for (std::size_t index = 0; index < extra_competitors.size(); ++index)
            {
                write_section_boundary();
                write_background_repeated(fltx_background, '-', comparison_bits_group_width);
            ...
            write_section_boundary();
            write_background_repeated(fltx_background, '-', preferred_reference_width + 2);
            write_border_join(fltx_background);
            write_background_repeated(fltx_background, '-', comparison_bits_group_width);
            write_border_join(fltx_background);
            write_background_repeated(fltx_background, '-', domain_single_group_width + 2);
        ...
        void write_colored_benchmark_metric(std::string_view text, std::string_view color)
        {
            const int padding = benchmark_speed_width - static_cast<int>(text.size());
            if (padding > 0)
                write_repeated(' ', padding);
            ...
            bool leading_section_boundary = false)
        {
            ...
            {
                write_accuracy_pair(accuracy, background, {}, leading_section_boundary);
                write_domain_single(accuracy.domain_score, background, background);
            }
            else
            {
                begin_cell(background, {}, leading_section_boundary);
                write_metric_text("-", bits_width);
                stream << ' ';
                write_metric_text("-", bits_width);
                end_cell(background);
                begin_cell(background, background);
                write_metric_text("-", domain_width);
                end_cell(background);
            }
            write_benchmark_single(benchmark.ns_per_iter, background, background);
            write_special_single(special_values, background, background);
        }
        ...
        {
            begin_cell(background, background);
            write_colored_benchmark_metric(format_metrics_ratio(ratio), color_for_speed_ratio(ratio));
            end_cell(background);
        }
            ...
            bool leading_section_boundary = false)
        {
            ...
            if (has_accuracy_data(fltx_accuracy) && has_accuracy_data(competitor_accuracy))
            {
                write_comparison_bits_pair(
                    ...
                    background,
                    leading_section_boundary);
                write_comparison_domain(
                    domain_gap(fltx_accuracy, competitor_accuracy),
                    background);
            }
            else
            {
            ...
            bool leading_section_boundary = false)
        {
            ...
            write_center_cell("domain", domain_width, background, background);
            write_center_cell("speed", benchmark_speed_width, background, background);
        }
            ...
            bool leading_section_boundary = false)
        {
            ...
            write_center_cell("score", domain_width, background, background);
            write_center_cell("ratio", benchmark_speed_width, background, background);
        }
        ...
        void write_header(std::string_view title, const metrics_console_report_summary& summary)
        {
            ...
            {
                begin_report_line();
                stream << "average precision samples: "
            ...
            write_center_cell(primary_backend_name, raw_backend_group_width, primary_competitor_background, {}, true);
            for (std::string_view competitor_name : extra_competitors)
            {
                const std::string backend_name = metrics_comparison_name(competitor_name);
                write_center_cell(
                    backend_name,
                    raw_backend_group_width,
                    extra_competitor_background,
                    {},
                    true);
            }
            write_center_cell(fltx_backend, raw_backend_group_width, fltx_background, {}, true);
            const std::string primary_comparison_name = "VS " + metrics_comparison_name(primary_competitor);
            write_center_cell(primary_comparison_name, comparison_group_width, fltx_background, {}, true);
            for (std::string_view competitor_name : extra_competitors)
            {
                const std::string comparison_name = "VS " + metrics_comparison_name(competitor_name);
                write_center_cell(comparison_name, comparison_group_width, fltx_background, {}, true);
            }
            write_final_superheader_cell(
                "VS preferred available reference",
                preferred_reference_group_width,
                fltx_background,
                {},
                true);
            write_section_boundary();
            ...
            write_center_cell("bits accurate", pair_group_width, primary_competitor_background, {}, true);
            write_center_cell("domain", domain_single_group_width, primary_competitor_background, primary_competitor_background);
            write_center_cell("bench", benchmark_single_group_width, primary_competitor_background, primary_competitor_background);
            write_center_cell("Inf/", special_single_group_width, primary_competitor_background, primary_competitor_background);
            for (std::size_t remaining = extra_competitors.size(); remaining > 0; --remaining)
            {
                write_center_cell("bits accurate", pair_group_width, extra_competitor_background, {}, true);
                write_center_cell("domain", domain_single_group_width, extra_competitor_background, extra_competitor_background);
                write_center_cell("bench", benchmark_single_group_width, extra_competitor_background, extra_competitor_background);
                write_center_cell("Inf/", special_single_group_width, extra_competitor_background, extra_competitor_background);
            }
            write_center_cell("bits accurate", pair_group_width, fltx_background, {}, true);
            write_center_cell("domain", domain_single_group_width, fltx_background, fltx_background);
            write_center_cell("bench", benchmark_single_group_width, fltx_background, fltx_background);
            write_center_cell("Inf/", special_single_group_width, fltx_background, fltx_background);
            ...
            begin_report_line();
            write_center_cell("", operation_column_width, metadata_background, {}, true);
            write_pair_subheader(primary_competitor_background, {}, true);
            write_domain_single_subheader(primary_competitor_background, primary_competitor_background);
            write_benchmark_single_subheader(primary_competitor_background, primary_competitor_background);
            write_center_cell("NaN", special_single_group_width, primary_competitor_background, primary_competitor_background);
            for (std::size_t index = 0; index < extra_competitors.size(); ++index)
            {
                write_pair_subheader(extra_competitor_background, {}, true);
                write_domain_single_subheader(extra_competitor_background, extra_competitor_background);
                write_benchmark_single_subheader(extra_competitor_background, extra_competitor_background);
                write_center_cell("NaN", special_single_group_width, extra_competitor_background, extra_competitor_background);
            }
            write_pair_subheader(fltx_background, {}, true);
            write_domain_single_subheader(fltx_background, fltx_background);
            write_benchmark_single_subheader(fltx_background, fltx_background);
            write_center_cell("NaN", special_single_group_width, fltx_background, fltx_background);
        ...
        const std::vector<std::string_view>& extra_competitor_names = {})
    {
        ...
        out << ','
            << record.fltx_accuracy.sample_count << ',';
        write_csv_text(out, record.competitor_name);
        out << ','
            << (record.competitor_supported ? "yes" : "no") << ','
            << record.fltx_accuracy.worst_bits << ','
            << record.fltx_accuracy.mean_bits << ','
            << record.fltx_accuracy.domain_score << ','
            << format_special_correctness(record.fltx_special_values) << ',';
        if (record.competitor_supported)
        {
            out << record.competitor_accuracy.worst_bits << ','
                << record.competitor_accuracy.mean_bits << ','
                << record.competitor_accuracy.domain_score << ','
                << format_special_correctness(record.competitor_special_values) << ','
                << precision_gap_bits(record) << ','
                << domain_gap(record.fltx_accuracy, record.competitor_accuracy) << ','
                << record.fltx_benchmark.ns_per_iter << ','
                << record.competitor_benchmark.ns_per_iter << ','
                << speed_ratio(record);
        }
        else
        {
        ...
        if (preferred.supported && preferred.accuracy != nullptr && preferred.benchmark != nullptr)
        {
            out << ',';
            write_csv_text(out, preferred.name);
            out << ','
                << precision_mean_advantage_bits(record.fltx_accuracy, *preferred.accuracy) << ','
                << precision_worst_advantage_bits(record.fltx_accuracy, *preferred.accuracy) << ','
                << domain_gap(record.fltx_accuracy, *preferred.accuracy) << ','
                << speed_ratio(record.fltx_benchmark, *preferred.benchmark);
        }
        else
        {
            out << ",-,-,-,-,-";
        ...
        for (std::string_view name : extra_competitor_names)
        {
            ...
            {
                out << ','
                    << competitor->accuracy.worst_bits << ','
                    << competitor->accuracy.mean_bits << ','
                    << competitor->accuracy.domain_score << ','
                    << format_special_correctness(competitor->special_values) << ','
                    << precision_gap_bits(record.fltx_accuracy, competitor->accuracy) << ','
                    << domain_gap(record.fltx_accuracy, competitor->accuracy) << ','
                    << competitor->benchmark.ns_per_iter << ','
                    << speed_ratio(record.fltx_benchmark, competitor->benchmark);
            }
            else
            {
                out << ",-,-,-,-,-,-,-,-";
