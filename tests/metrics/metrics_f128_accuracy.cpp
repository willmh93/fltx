...
#define FLTX_F128_UNARY_ACCURACY(NAME, TAGS, REQUIRED_BITS, SAMPLES, CALL) \
    TEST_CASE("f128 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " #NAME " precision", "[metrics][precision][accuracy][fltx][f128][primary]" TAGS) \
    { \
...
#define FLTX_F128_BINARY_ACCURACY(NAME, TAGS, REQUIRED_BITS, SAMPLES, CALL, REF_CALL) \
    TEST_CASE("f128 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " #NAME " precision", "[metrics][precision][accuracy][fltx][f128][primary]" TAGS) \
    { \
        if (!bl::test::metrics::metrics_case_phase_enabled("precision")) { SUCCEED("metrics precision phase not selected"); return; } \
        const bool enforce_assertions = bl::test::metrics::metrics_case_assertions_enabled(); \
...
#define FLTX_F128_BINARY_BOOL_ACCURACY(NAME, LABEL, TAGS, REQUIRED_BITS, SAMPLES, CALL, REF_CALL) \
    TEST_CASE("f128 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " LABEL " precision", "[metrics][precision][accuracy][fltx][f128][primary]" TAGS) \
    { \
        if (!bl::test::metrics::metrics_case_phase_enabled("precision")) { SUCCEED("metrics precision phase not selected"); return; } \
        const bool enforce_assertions = bl::test::metrics::metrics_case_assertions_enabled(); \
        const auto samples = SAMPLES(); \
        const auto record = run_binary_bool_case<FLTX_PRIMARY_CASE_QDPP_EXTRA_SUPPORTED(NAME)>( \
            ...
            [](const auto& x, const auto& y) { return REF_CALL(x, y); }, \
            false, enforce_assertions); \
        bl::test::metrics::write_metrics_case_report("f128 primary precision: " LABEL, record); \
        if (enforce_assertions) check_competitor_slack(record); \
    }

#define FLTX_F128_TERNARY_ACCURACY(NAME, TAGS, REQUIRED_BITS, SAMPLES, CALL, REF_CALL) \
    TEST_CASE("f128 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " #NAME " precision", "[metrics][precision][accuracy][fltx][f128][primary]" TAGS) \
    { \
        if (!bl::test::metrics::metrics_case_phase_enabled("precision")) { SUCCEED("metrics precision phase not selected"); return; } \
        const bool enforce_assertions = bl::test::metrics::metrics_case_assertions_enabled(); \
...
#define FLTX_F128_UNARY_INT_ACCURACY(NAME, TAGS, REQUIRED_BITS, SAMPLES, CALL) \
    TEST_CASE("f128 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " #NAME " precision", "[metrics][precision][accuracy][fltx][f128][primary]" TAGS) \
    { \
        if (!bl::test::metrics::metrics_case_phase_enabled("precision")) { SUCCEED("metrics precision phase not selected"); return; } \
        const bool enforce_assertions = bl::test::metrics::metrics_case_assertions_enabled(); \
...
#define FLTX_F128_UNARY_INTEGER_ACCURACY(NAME, TAGS, REQUIRED_BITS, SAMPLES, CALL, REF_CALL) \
    TEST_CASE("f128 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " #NAME " precision", "[metrics][precision][accuracy][fltx][f128][primary]" TAGS) \
    { \
        if (!bl::test::metrics::metrics_case_phase_enabled("precision")) { SUCCEED("metrics precision phase not selected"); return; } \
        const bool enforce_assertions = bl::test::metrics::metrics_case_assertions_enabled(); \
...
#define FLTX_F128_FREXP_ACCURACY(NAME, TAGS, REQUIRED_BITS, SAMPLES, CALL) \
    TEST_CASE("f128 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " #NAME " precision", "[metrics][precision][accuracy][fltx][f128][primary]" TAGS) \
    { \
        if (!bl::test::metrics::metrics_case_phase_enabled("precision")) { SUCCEED("metrics precision phase not selected"); return; } \
        const bool enforce_assertions = bl::test::metrics::metrics_case_assertions_enabled(); \
...
FLTX_F128_PRIMARY_CASES(
    FLTX_F128_UNARY_ACCURACY,
    FLTX_F128_BINARY_ACCURACY,
    FLTX_F128_TERNARY_ACCURACY,
    FLTX_F128_UNARY_INT_ACCURACY,
    FLTX_F128_UNARY_INTEGER_ACCURACY,
    FLTX_F128_FREXP_ACCURACY)

FLTX_F128_PRIMARY_COMPARISON_CASES(FLTX_F128_BINARY_BOOL_ACCURACY)

#undef FLTX_F128_UNARY_ACCURACY
#undef FLTX_F128_BINARY_ACCURACY
#undef FLTX_F128_BINARY_BOOL_ACCURACY
#undef FLTX_F128_TERNARY_ACCURACY
#undef FLTX_F128_UNARY_INT_ACCURACY
#undef FLTX_F128_UNARY_INTEGER_ACCURACY
#undef FLTX_F128_FREXP_ACCURACY
