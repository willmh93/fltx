...
#define FLTX_F128_UNARY_BENCH(NAME, TAGS, REQUIRED_BITS, SAMPLES, CALL) \
    ...
    { \
        ...
        bl::test::metrics::write_metrics_case_report("f128 primary benchmark: " #NAME, record); \
        check_benchmark_claim_is_meaningful(record); \
    }

#define FLTX_F128_BINARY_BENCH(NAME, TAGS, REQUIRED_BITS, SAMPLES, CALL, REF_CALL) \
    TEST_CASE("f128 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " #NAME " benchmark", "[metrics][bench][fltx][f128][primary]" TAGS) \
    { \
        if (!bl::test::metrics::metrics_case_phase_enabled("bench")) { SUCCEED("metrics benchmark phase not selected"); return; } \
        const scoped_benchmark_sample_count benchmark_samples{ #NAME }; \
...
#define FLTX_F128_BINARY_BOOL_BENCH(NAME, LABEL, TAGS, REQUIRED_BITS, SAMPLES, CALL, REF_CALL) \
    ...
    { \
        ...
        const auto record = run_binary_bool_case<FLTX_PRIMARY_CASE_QDPP_EXTRA_SUPPORTED(NAME)>( \
            ...
            [](const auto& x, const auto& y) { return REF_CALL(x, y); }, \
            true, false, domain_ideal_bits, false); \
        bl::test::metrics::write_metrics_case_report("f128 primary benchmark: " LABEL, record); \
        check_benchmark_claim_is_meaningful(record); \
    }

#define FLTX_F128_TERNARY_BENCH(NAME, TAGS, REQUIRED_BITS, SAMPLES, CALL, REF_CALL) \
    TEST_CASE("f128 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " #NAME " benchmark", "[metrics][bench][fltx][f128][primary]" TAGS) \
    { \
        if (!bl::test::metrics::metrics_case_phase_enabled("bench")) { SUCCEED("metrics benchmark phase not selected"); return; } \
        const scoped_benchmark_sample_count benchmark_samples{ #NAME }; \
...
#define FLTX_F128_UNARY_INT_BENCH(NAME, TAGS, REQUIRED_BITS, SAMPLES, CALL) \
    TEST_CASE("f128 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " #NAME " benchmark", "[metrics][bench][fltx][f128][primary]" TAGS) \
    { \
        if (!bl::test::metrics::metrics_case_phase_enabled("bench")) { SUCCEED("metrics benchmark phase not selected"); return; } \
        const scoped_benchmark_sample_count benchmark_samples{ #NAME }; \
...
#define FLTX_F128_UNARY_INTEGER_BENCH(NAME, TAGS, REQUIRED_BITS, SAMPLES, CALL, REF_CALL) \
    TEST_CASE("f128 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " #NAME " benchmark", "[metrics][bench][fltx][f128][primary]" TAGS) \
    { \
        if (!bl::test::metrics::metrics_case_phase_enabled("bench")) { SUCCEED("metrics benchmark phase not selected"); return; } \
        const scoped_benchmark_sample_count benchmark_samples{ #NAME }; \
...
#define FLTX_F128_FREXP_BENCH(NAME, TAGS, REQUIRED_BITS, SAMPLES, CALL) \
    TEST_CASE("f128 primary " FLTX_PRIMARY_CASE_ORDER(NAME) " " #NAME " benchmark", "[metrics][bench][fltx][f128][primary]" TAGS) \
    { \
        if (!bl::test::metrics::metrics_case_phase_enabled("bench")) { SUCCEED("metrics benchmark phase not selected"); return; } \
        const scoped_benchmark_sample_count benchmark_samples{ #NAME }; \
...
FLTX_F128_PRIMARY_CASES(
    FLTX_F128_UNARY_BENCH,
    FLTX_F128_BINARY_BENCH,
    FLTX_F128_TERNARY_BENCH,
    FLTX_F128_UNARY_INT_BENCH,
    FLTX_F128_UNARY_INTEGER_BENCH,
    FLTX_F128_FREXP_BENCH)

FLTX_F128_PRIMARY_COMPARISON_CASES(FLTX_F128_BINARY_BOOL_BENCH)

#undef FLTX_F128_UNARY_BENCH
#undef FLTX_F128_BINARY_BENCH
#undef FLTX_F128_BINARY_BOOL_BENCH
#undef FLTX_F128_TERNARY_BENCH
#undef FLTX_F128_UNARY_INT_BENCH
#undef FLTX_F128_UNARY_INTEGER_BENCH
#undef FLTX_F128_FREXP_BENCH
